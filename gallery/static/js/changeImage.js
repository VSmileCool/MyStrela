...
function moveText() {
  ...
  document.getElementById("textOnImage").style =
    "z-index: 1;position: absolute; visibility: visible; margin-top: " +
    moveOnY +
    "%; margin-left: " +
    moveOnX +
    "%;";
...
function setHrefToCropButton() {
  document
    .getElementById("goToCrop")
    .setAttribute(
      "href",
      "crop_image/?photo=" + pathToImage + "&id={{photoid}}",
    );
  alert(pathToImage);
}
...
function cropImage() {
  ...
  $.ajax({
    ...
    data: {
      x: document.getElementById("dataX").value,
      ...
      width: document.getElementById("dataWidth").value,
...
function saveImage() {
  $.ajax({
    ...
    type: "GET", // http method
    data: {
      brightness: brightnessInPercentage,
      ...
      id: document.getElementById("aPhotoId").textContent,
      photo: document.getElementById("imageforchanging").src,
    }, // data sent with the get request
...
function setStyleToImage() {
  ...
  document.getElementById("imageforchanging").style =
    centered + rotation + filters;
}
